...
var localConfigPath = argv.c || Path.join(rootPath, 'config.local.yaml');
...
module.exports.resolve = function(pathPropName, paths) {
	var args = [rootPath, _.get(setting, pathPropName)];
...
function load() {
	...
	log.info(`Read ${Path.resolve(__dirname, '../config.yaml')}`);
	_.assign(setting, {
			/** @name rootPath
			* @memberof setting
			*/
	...
	var config2 = Path.resolve(rootPath, 'config.yaml');
	if (setting.dependencyMode && fs.existsSync(config2)) {
		log.info(`Read ${config2}`);
