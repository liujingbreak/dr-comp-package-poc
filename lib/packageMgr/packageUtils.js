...
function findNodeEntryFiles(recipePackageJson, eachCallback) {
	...
	_.forOwn(pj.dependencies, function(version, name) {
		...
		var json = JSON.parse(fs.readFileSync(packageJson, 'utf-8'));
		if (json.main) {
			eachCallback(name, entryPath, parsedName, json);
		}
	});
}
...
function findBrowserPackageByType(types, callback) {
	types = [].concat(types);
	var recipe = Path.resolve(config().rootPath, config().recipeFolder, 'package.json');
	findBrowserEntryFiles(recipe, function(name, entryPath, parsedName, pkJson, packagePath) {
...
function findNodePackageByType(types, callback) {
	types = [].concat(types);
	var recipe = Path.resolve(config().rootPath, config().recipeFolder, 'package.json');
	findNodeEntryFiles(recipe, function(name, entryPath, parsedName, pkJson) {
		var packageType = pkJson.dr ? (pkJson.dr.type ? pkJson.dr.type : null) : null;
		if (_.includes(types, packageType)) {
			callback(name, entryPath, parsedName, pkJson);
		}
	});
}
