...
function readPackages(dontLoad) {
	...
	putil.findNodePackageByType('*',
		function(moduleName, entryPath, parsedName, pjson, packagePath) {
			var types = [].concat(_.get(pjson, 'dr.type'));
			var isServer = _.includes(types, 'server');
			if (isServer) {
				packages.push({
				...
				var pk = new Package({
					...
					path: packagePath,
					priority: isServer ? pjson.dr.serverPriority : _.get(pjson, 'dr.builderPriority')
				});
