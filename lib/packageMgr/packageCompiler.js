...
function listPackages() {
	...
	return Promise.coroutine(function*() {
		yield priorityHelper.orderPackages(packages, pk => {
			...
			list.push({
				pk: pk,
				desc: util.format('%d. %s %scompile priority: %s', idx, pk.longName, gap.join(''), pk.priority),
			});
		});
		return list;
	})();
...
function _listPackages(onEachPackage) {
	var packages = [];
	packageUtils.findNodePackageByType('builder', (name, entryPath, parsedName, pkJson, packagePath) => {
