...
InstallManager.prototype = {
	...
	printDep: function() {
		var allDepJson = {};
		log.info('------ components dependencies -------');

		var mainPkjson = fs.readFileSync(Path.resolve(config().rootPath, 'package.json'));
		mainPkjson = JSON.parse(mainPkjson);
		var mainDeps = _.assign({}, mainPkjson.dependencies, mainPkjson.devDependencies);

		_.forOwn(this.srcDeps, (versionList, name) => {
			log.info(name + ' <- ' + _.map(versionList, item => item.ver + ' ' + item.by).join(', '));
			if (_.has(mainDeps, name))
				log.info('\tis duplicate with main package.json', name);
			else
				allDepJson[name] = versionList[0].ver;
		});
		mkdirp.sync(config().destDir);
		var file = Path.join(config().destDir, 'component-dependencies.json');
		fs.writeFileSync(file, JSON.stringify(allDepJson, null, '  '));
		log.info('%s is written.', file);
	}
};
