...
function symbolicLinkPackages(destDir, recipeFile) {
	return through.obj(function(file, encoding, callback) {
		...
		Promise.coroutine(function*() {
			...
			try {
				fs.accessSync(newPath, fs.F_OK); // if file exits
				var stat = fs.lstatSync(newPath);
				if (stat.isSymbolicLink() && stat.mtime.getTime() < file.stat.mtime.getTime()) {
					log.info('Overwrite symbolic link %s', newPath);
					yield fs.unlink(newPath);
