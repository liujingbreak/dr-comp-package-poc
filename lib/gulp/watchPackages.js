	...
	var onChange = _.debounce(function() {
		...
		.then(function() {
			...
			if (_.get(currDirty, '__all') === true) {
				...
				log.debug('changed packages: %s %s', argv.p, argv['only-js'] ? 'only js' : '');
			}

			// var processArgs = ['compile'];
			// _.each(argv.p, name => {
			// 	processArgs.push('-p');
			// 	processArgs.push(name);
			// });
			// if (argv['only-css'])
			// 	processArgs.push('--only-css');
			// delete argv.p;
			// delete argv['only-css'];
			// return buildUtils.promisifySpawn('gulp', processArgs)
			// .then(() => {
			// 	log.info('done');
			// });
